...
namespace Location.Core.Infrastructure.Tests.Data.Repositories
{
    ...
    public class SettingRepositoryAdapterTests
    {
        ...
        public async Task DeleteAsync_WithExistingSetting_ShouldReturnSuccess()
        {
            ...
            _mockInnerRepository.Setup(x => x.GetByKeyAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(setting);
            _mockInnerRepository.Setup(x => x.DeleteAsync(It.IsAny<Setting>(), CancellationToken.None))
                .Returns(Task.CompletedTask);
            ...
            _mockInnerRepository.Verify(x => x.GetByKeyAsync("test_key", It.IsAny<CancellationToken>()), Times.Once);
            _mockInnerRepository.Verify(x => x.DeleteAsync(setting, CancellationToken.None), Times.Once);
        }

        [Test]
        public async Task DeleteAsync_WithNonExistentKey_ShouldReturnFailure()
        {
            // Arrange
            _mockInnerRepository.Setup(x => x.GetByKeyAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync((Setting?)null);
        ...
        public async Task UpsertAsync_WithValidSetting_ShouldReturnSuccess()
        {
            // Arrange
            var setting = TestDataBuilder.CreateValidSetting();
            var upsertedSetting = TestDataBuilder.CreateValidSetting(key: setting.Key, value: setting.Value, description: setting.Description);
