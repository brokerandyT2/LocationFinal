...
public class FunctionGeneratorService
{
    ...
    private string GenerateAuthEndpoints(StringBuilder sb, AssemblyInfo assemblyInfo)
    {
        ...
        sb.AppendLine("        try");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogInformation(\"Processing QR request for {Vertical}\", \"{assemblyInfo.Vertical}\");");
        sb.AppendLine();
        sb.AppendLine("            var requestBody = await JsonSerializer.DeserializeAsync<RegistrationRequest>(req.Body);");
        sb.AppendLine("            var appType = \"{assemblyInfo.Vertical}\"; // Extract from route");
        sb.AppendLine();
        ...
        sb.AppendLine("            await UpsertUserRegistrationAsync(requestBody.Email, requestBody.AppGuid, appTypeId);");
        sb.AppendLine();
        sb.AppendLine("            _logger.LogInformation(\"QR request processed for user: {Email}, appType: {AppType}\", requestBody.Email, appType);");
        sb.AppendLine();
        ...
        sb.AppendLine("        try");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogInformation(\"Processing email verification for {Vertical}\", \"{assemblyInfo.Vertical}\");");
        sb.AppendLine();
        sb.AppendLine("            var requestBody = await JsonSerializer.DeserializeAsync<RegistrationRequest>(req.Body);");
        ...
        sb.AppendLine("            await response.WriteStringAsync(JsonSerializer.Serialize(new { ");
        sb.AppendLine("                status = \"success\", ");
        sb.AppendLine("                message = \"Email verified\",");
        sb.AppendLine("                appTypeId = appTypeId");
        sb.AppendLine("            }));");
        sb.AppendLine("            return response;");
        sb.AppendLine("        }");
        sb.AppendLine("        catch (Exception ex)");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogError(ex, \"Email verification failed\");");
        sb.AppendLine("            var errorResponse = req.CreateResponse(HttpStatusCode.InternalServerError);");
        sb.AppendLine("            await errorResponse.WriteStringAsync(JsonSerializer.Serialize(new { status = \"error\", message = ex.Message }));");
        sb.AppendLine("            return errorResponse;");
        sb.AppendLine("        }");
        ...
        sb.AppendLine("        try");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogInformation(\"Generating QR code for {Vertical}\", \"{assemblyInfo.Vertical}\");");
        sb.AppendLine();
        sb.AppendLine("            var requestBody = await JsonSerializer.DeserializeAsync<RegistrationRequest>(req.Body);");
        sb.AppendLine("            var appType = \"{assemblyInfo.Vertical}\";");
        sb.AppendLine();
        ...
        sb.AppendLine("            await response.WriteStringAsync(JsonSerializer.Serialize(new { ");
        sb.AppendLine("                status = \"success\", ");
        sb.AppendLine("                qrCode = \"generated-qr-data\",");
        sb.AppendLine("                appTypeId = appTypeId");
        sb.AppendLine("            }));");
        sb.AppendLine("            return response;");
        sb.AppendLine("        }");
        sb.AppendLine("        catch (Exception ex)");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogError(ex, \"QR generation failed\");");
        sb.AppendLine("            var errorResponse = req.CreateResponse(HttpStatusCode.InternalServerError);");
        sb.AppendLine("            await errorResponse.WriteStringAsync(JsonSerializer.Serialize(new { status = \"error\", message = ex.Message }));");
        sb.AppendLine("            return errorResponse;");
        sb.AppendLine("        }");
        ...
        sb.AppendLine("        try");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogInformation(\"Processing QR scan for {Vertical}\", \"{assemblyInfo.Vertical}\");");
        sb.AppendLine();
        sb.AppendLine("            var requestBody = await JsonSerializer.DeserializeAsync<RegistrationRequest>(req.Body);");
        sb.AppendLine("            var appType = \"{assemblyInfo.Vertical}\";");
        sb.AppendLine();
        ...
        sb.AppendLine("            await response.WriteStringAsync(JsonSerializer.Serialize(new { ");
        sb.AppendLine("                status = \"success\", ");
        sb.AppendLine("                message = \"QR scanned successfully\",");
        sb.AppendLine("                appTypeId = appTypeId");
        sb.AppendLine("            }));");
        sb.AppendLine("            return response;");
        sb.AppendLine("        }");
        sb.AppendLine("        catch (Exception ex)");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogError(ex, \"QR scan processing failed\");");
        sb.AppendLine("            var errorResponse = req.CreateResponse(HttpStatusCode.InternalServerError);");
        sb.AppendLine("            await errorResponse.WriteStringAsync(JsonSerializer.Serialize(new { status = \"error\", message = ex.Message }));");
        sb.AppendLine("            return errorResponse;");
        sb.AppendLine("        }");
        ...
        sb.AppendLine("        try");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogInformation(\"Processing manual restore for {Vertical}\", \"{assemblyInfo.Vertical}\");");
        sb.AppendLine();
        sb.AppendLine("            var requestBody = await JsonSerializer.DeserializeAsync<RegistrationRequest>(req.Body);");
        sb.AppendLine("            var appType = \"{assemblyInfo.Vertical}\";");
        sb.AppendLine();
        ...
        sb.AppendLine("            await response.WriteStringAsync(JsonSerializer.Serialize(new { ");
        sb.AppendLine("                status = \"success\", ");
        sb.AppendLine("                message = \"Manual restore completed\",");
        sb.AppendLine("                appTypeId = appTypeId");
        sb.AppendLine("            }));");
        sb.AppendLine("            return response;");
        sb.AppendLine("        }");
        sb.AppendLine("        catch (Exception ex)");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogError(ex, \"Manual restore processing failed\");");
        sb.AppendLine("            var errorResponse = req.CreateResponse(HttpStatusCode.InternalServerError);");
        sb.AppendLine("            await errorResponse.WriteStringAsync(JsonSerializer.Serialize(new { status = \"error\", message = ex.Message }));");
        sb.AppendLine("            return errorResponse;");
        sb.AppendLine("        }");
        ...
        sb.AppendLine("        try");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogInformation(\"Sending recovery QR for {Vertical}\", \"{assemblyInfo.Vertical}\");");
        sb.AppendLine();
        sb.AppendLine("            var requestBody = await JsonSerializer.DeserializeAsync<RegistrationRequest>(req.Body);");
        sb.AppendLine("            var appType = \"{assemblyInfo.Vertical}\";");
        sb.AppendLine();
        ...
        sb.AppendLine("            await response.WriteStringAsync(JsonSerializer.Serialize(new { ");
        sb.AppendLine("                status = \"success\", ");
        sb.AppendLine("                message = \"Recovery QR sent\",");
        sb.AppendLine("                appTypeId = appTypeId");
        sb.AppendLine("            }));");
        sb.AppendLine("            return response;");
        sb.AppendLine("        }");
        sb.AppendLine("        catch (Exception ex)");
        sb.AppendLine("        {");
        sb.AppendLine("            _logger.LogError(ex, \"Recovery QR processing failed\");");
        sb.AppendLine("            var errorResponse = req.CreateResponse(HttpStatusCode.InternalServerError);");
        sb.AppendLine("            await errorResponse.WriteStringAsync(JsonSerializer.Serialize(new { status = \"error\", message = ex.Message }));");
        sb.AppendLine("            return errorResponse;");
        sb.AppendLine("        }");
        ...
        // Add the helper methods and data models
        GenerateRegistrationHelperMethods(sb);
        sb.AppendLine("}");

        return sb.ToString();
    }

    private void GenerateRegistrationHelperMethods(StringBuilder sb)
    {
        sb.AppendLine("    // Registration helper methods");
        ...
        sb.AppendLine("        command.Parameters.AddWithValue(\"@AppTypeName\", appTypeName);");
        sb.AppendLine();
        sb.AppendLine("        var result = await command.ExecuteScalarAsync();");
        sb.AppendLine("        var appTypeId = Convert.ToInt32(result);");
        sb.AppendLine();
        sb.AppendLine("        _logger.LogDebug(\"Upserted AppType: {AppTypeName} -> ID: {AppTypeId}\", appTypeName, appTypeId);");
        ...
        sb.AppendLine();
        sb.AppendLine("        using var command = new SqlCommand(query, connection);");
        sb.AppendLine("        command.Parameters.AddWithValue(\"@Email\", email);");
        sb.AppendLine("        command.Parameters.AddWithValue(\"@AppGuid\", appGuid);");
        sb.AppendLine("        command.Parameters.AddWithValue(\"@AppTypeId\", appTypeId);");
        sb.AppendLine();
        sb.AppendLine("        await command.ExecuteNonQueryAsync();");
        sb.AppendLine();
        sb.AppendLine("        _logger.LogDebug(\"Upserted UserRegistration: {Email}, AppTypeId: {AppTypeId}\", email, appTypeId);");
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    // Registration request model");
