...
public partial class UserOnboarding : ContentPage
{
    ...
    /// </summary>
    public UserOnboarding(IAlertService alertService)
    {
    ...
    private void GetSetting()
    {
        ...
        {
            WindDirection.Text = Location.Core.Resources.AppResources.TowardsWind.FirstCharToUpper();
        }
        else
        {
            WindDirection.Text = Location.Core.Resources.AppResources.WithWind.FirstCharToUpper();
        }
    }
    ...
    private async void save_Pressed(object sender, EventArgs e)
    {
        ...
        if (isValid && hasText)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (Location.Core.Resources.AppResources.ResourceManager.GetString("ErrorProcessingData") != null)
                {
                    errorMessage = Location.Core.Resources.AppResources.ResourceManager.GetString("ErrorProcessingData");
                }

                await DisplayAlert(Location.Core.Resources.AppResources.Error,
                                  $"{errorMessage}: {ex.Message}",
                                  Location.Core.Resources.AppResources.OK);
            }
            finally
            {
    ...
    private void WindDirectionSwitch_Toggled(object sender, ToggledEventArgs e)
    {
        ...
        WindDirection.Text = e.Value
            ? Location.Core.Resources.AppResources.TowardsWind.FirstCharToUpper()
            : Location.Core.Resources.AppResources.WithWind.FirstCharToUpper();
    }
