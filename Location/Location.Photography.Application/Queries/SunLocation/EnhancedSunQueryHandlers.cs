// Location.Photography.Application/Queries/SunLocation/EnhancedSunQueryHandlers.cs
...
namespace Location.Photography.Application.Queries.SunLocation
{
    ...
    public class GetMoonDataQueryHandler : IRequestHandler<GetMoonDataQuery, Result<MoonPhaseData>>
    {
        ...
        {
            var newMoonDate = new DateTime(2024, 1, 11);
        ...
        {
            return date.Date.AddHours(20.5 + (latitude / 15));
        }

        private DateTime? CalculateMoonSet(DateTime date, double latitude, double longitude)
        {
            return date.Date.AddHours(6.5 + (latitude / 15));
        }
        ...
        private MoonPosition CalculateMoonPosition(DateTime date, double latitude, double longitude)
        {
            ...
            {
                Azimuth = phase * 360,
                Elevation = 45 - Math.Abs(latitude) / 2,
                Distance = 384400,
                IsAboveHorizon = true
            };
        }
        ...
        {
            return -12.74 + 0.026 * Math.Abs(phase - 0.5) * 180;
        }
    }
